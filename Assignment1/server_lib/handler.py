...
class Server:
    ...
    def terminated(self): 
        # data : dict = json.load(open("config.json", "r"))
    ...
    def run(self):
        ...
        #     while threading.active_count() <= self.maximiumClient:
            connection, address = self.server.accept()
            data = json.loads(connection.recv(1024).decode())
            if self.aunthenticate(data):
            ...
            # if input() == '0':
            #     break
            # Check if old thread is still working
    ...
    def aunthenticate(self, data : dict):
        auth : bool
        meh = data["auth"]
        print(meh)
        if meh == "signup":
